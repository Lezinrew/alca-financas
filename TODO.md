...
**Status:** 🟡 Em progresso — P0-B (runtime único chatbot) e P0-D (docs mínimas) em curso; frente **auth/RLS/tenant** estável no código e nas migrations `00001`–`00004` (aplicar no Supabase de cada ambiente). **Admin:** migration `20260417000002` + código alinhados — aplicar migration em cada ambiente antes de usar purge em produção. Itens de 2026-04-17 abaixo não foram reverificados nesta atualização — apenas o texto e a data do cabeçalho foram alinhados; ver seção "Débito técnico" (Prioridade 5) para o backlog ainda não atacado.

### Concluído recentemente (código na `main`)

- **Pipeline de deploy corrigido (2026-08-09):** `.github/workflows/deploy-production.yml` apontava para `/var/www/alca-financas` (diretório que não existia mais no servidor — o projeto real está em `/apps/alca-financas`) e tentava subir o serviço `openclaw-bridge`, removido do `docker-compose.prod.yml` em 2026-07-26. Ambos corrigidos (`1f65f626`, `335c726b`, `8eb8298d`); documentação e scripts ativos atualizados para o caminho correto (`8c30b15e`). Decisão da migração FinanceOS/OFX formalizada em `docs/decisions/0008-financeos-migration-ofx-ssot.md`.
- **Redesign da tela de login (2026-07-30):** layout SaaS premium em duas colunas (`Login.tsx`, `LoginVisualPanel.tsx`, `DashboardMockup.tsx`), comportamento de auth preservado. `docs/LOGIN-UPGRADE-ENTREGA.md` descreve uma entrega anterior (2026-02-10) e não foi atualizado para esta versão.
