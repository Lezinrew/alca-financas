...
const AppShell = () => {
  ...
  return (
    <div className="flex h-screen bg-slate-50 dark:bg-[#0f172a]">
      ...
      {/* Sidebar */}
      <aside className={`bg-[#1a1d29] dark:bg-[#1a1d29] flex flex-col transition-all duration-300 ease-in-out h-full ${
        sidebarHidden 
          ? '-translate-x-full lg:translate-x-0 lg:w-16' 
          : 'translate-x-0 w-60'
      } fixed lg:relative z-40`}>
        {/* Logo */}
        <div className={`h-16 flex items-center justify-between border-b border-slate-700 ${
          sidebarHidden ? 'px-2 lg:justify-center' : 'px-6'
        }`}>
            ...
            return (
              <NavLink
                ...
                className={`flex items-center rounded-lg text-sm font-medium transition-colors ${
                  sidebarHidden ? 'justify-center px-2 py-2.5' : 'px-3 py-2.5'
                } ${
                  isActive
      ...
      {/* Main Content */}
      <div className={`flex-1 flex flex-col overflow-hidden bg-slate-50 dark:bg-[#0f172a] transition-all duration-300 ${
        sidebarHidden ? 'lg:ml-0' : 'lg:ml-0'
      }`}>
        {/* Top Bar */}
